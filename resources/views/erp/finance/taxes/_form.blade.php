...
<div class="grid grid-cols-1 gap-5">
    ...
    <div>
        ...
        <select name="type" id="type" required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand sm:text-sm">
            @foreach (\App\Models\Tax::TYPES as $value => $label)
                <option value="{{ $value }}" @selected(old('type', $tax->type ?? '') === $value)>{{ $label }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input type="number" step="0.01" min="0.01" max="100" name="rate" id="rate" value="{{ old('rate', $tax->rate ?? '') }}" required
               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand sm:text-sm">
    ...
    <div class="flex items-center gap-2">
        ...
        <input type="checkbox" name="is_active" id="is_active" value="1"
               @checked(old('is_active', $tax->is_active ?? true))
    ...
    <div>
        ...
        <textarea name="description" id="description" rows="3"
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand sm:text-sm">{{ old('description', $tax->description ?? '') }}</textarea>
...
<div class="mt-6 flex items-center gap-3">
    ...
    <a href="{{ route('finance.taxes') }}" class="text-sm text-gray-500 hover:text-gray-700">Cancel</a>
</div>
