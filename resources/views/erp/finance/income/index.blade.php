<x-layouts.erp title="Income">
    <div class="max-w-6xl">
        <x-erp.flash />

        <div class="flex items-center justify-between mb-4">
            <p class="text-sm text-gray-500">Record income transactions with auto-numbered receipts.</p>
            @can('finance.manage')
                <a href="{{ route('finance.income.create') }}" class="inline-flex items-center rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark">
                    Record Income
                </a>
            @endcan
        </div>
                        ...
                        <tr>
                            ...
                            @can('finance.manage')
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('finance.income.edit', $transaction) }}" class="text-brand hover:text-brand-dark font-medium">Edit</a>
                                    <form action="{{ route('finance.income.destroy', $transaction) }}" method="POST" class="inline" onsubmit="return confirm('Delete this income record?');">
                                        @csrf
                                        @method('DELETE')
